...
Pester\Describe "Manifest" {
    ...
    Pester\It "has a valid guid" {
        $ManifestHash.Guid | Should Be "<%=$PLASTER_Guid1%>"
    }
    ...
    Pester\It 'has a valid project Uri' {
        $ManifestHash.PrivateData.Values.ProjectUri | Should Be "https://github.com/<%=$PLASTER_PARAM_GithubUserName%>/<%=$PLASTER_PARAM_ModuleName%>"
    }
